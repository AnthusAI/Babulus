    ...
    const loadTimeline = async () => {
      try {
        ...
        const data = (await response.json()) as {
          audio?: { tracks?: Array<{ kind?: string; clips?: Array<{ id: string; startSec: number; durationSec: number; src?: string }> }> };
        };
        if (!isActive) return;
        const tracks = data.audio?.tracks ?? [];
        const narration =
          tracks.find((track) => track.kind === 'narration') ?? tracks[0];
  ...
  useEffect(() => {
    ...
    const loadIndex = async () => {
      try {
        ...
        const data = (await response.json()) as { compositions?: Array<{ id: string; audio?: string | null }> };
        if (!isActive) return;
      ...
      <PreviewPlayer
        ...
        onPlayStateChange={(playing) => {
          ...
          if (timelineClips.length > 0) {
            syncAudioToTime(lastTimeRef.current);
            if (!playing) {
              audio.pause();
            }
            return;
