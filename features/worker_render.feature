Feature: Render Worker Job Processing
  ...
  Scenario: Process render job with missing generation run
    ...
    When the worker processes the render job
    Then the job should fail with "Generation run not found"
  ...
  Scenario: Process render job with missing script artifact
    ...
    When the worker processes the render job
    Then the job should fail with "No script artifact found"
    And the job status should be updated to "failed"

  Scenario: Emit progress events during rendering
    Given a claimed render job with valid generation run
    When the worker processes the render job
    Then a JobEvent should be emitted with message "Fetching generation run..."
    And a JobEvent should be emitted with message "Downloading artifacts..."
    And a JobEvent should be emitted with message "Rendering video..."
    And a JobEvent should be emitted with message "Uploading video..."
    And a JobEvent should be emitted with message "Creating render run..."
    And a JobEvent should be emitted with message "Render complete!"
  ...
  Scenario: Retry failed render job when retryCount < maxRetries
    ...
    When the job fails with error "Playwright crashed"
    Then the job should be re-queued
    And the retryCount should be 1
    And the failureReason should contain "Retry 1/3: Playwright crashed"
